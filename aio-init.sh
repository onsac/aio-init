...
check_step 1
if [ "$?" -eq "0" ]; then
   yum update -y && set_step 1 "update OK" || echo "1 - update failed" && exit 1
fi

check_step 2
if [ "$?" -eq "0" ]; then
   yum install -y git && set_step 2 "Install git OK" || echo "2 - Install git failed" && exit 1
fi
...
check_step 10
if [ "$?" -eq "0" ]; then
   nvm install 11 && set_step 10 "install nodejs OK" || echo "10 - install nodejs failed" && exit 1
fi

check_step 11
if [ "$?" -eq "0" ]; then
   nvm alias default 11 && set_step 11 "set alias OK" || echo "11 - set alias failed" && exit 1
fi
...
check_step 13
if [ "$?" -eq "0" ]; then
   git config —-global credential.helper store && set_step 13 "git config OK" || echo "13 - git config failed" && exit 1
fi
...
check_step 33
if [ "$?" -eq "0" ]; then
   npm install -g pm2 1>/dev/null && set_step 33 "install pm2 OK" || echo "33 - install pm2 failed" && exit 1
fi

check_step 34
if [ "$?" -eq "0" ]; then
   pm2 install pm2-logrotate 1>/dev/null && set_step 34 "install pm2-logrotate OK" || echo "34 - install pm2-logrotate failed" && exit 1
fi

check_step 35
if [ "$?" -eq "0" ]; then
   pm2 link svnfgywh46k3e51 dlrucnwgh2w7bzs 1>/dev/null && set_step 35 "install pm2-link OK" || echo "35 - install pm2-link failed" && exit 1
fi
...
check_step 37
if [ "$?" -eq "0" ]; then
   pw2 | sudo -S $STARTUP 1>/dev/null && set_step 37 "set pm2-startup OK" || echo "37 - set pm2-startup failed" && exit 1
fi
